...
def main():
    st.set_page_config(page_title="Customer Analyser", layout="wide")

    st.sidebar.title("Customer Analyser")
    st.sidebar.markdown(
        """Diese Anwendung ermöglicht die interaktive Datenanalyse von [Online Retail](https://www.kaggle.com/vijayuv/onlineretail) Daten.
  ...
  """
    ...
    col1.markdown(
        """Für die geladenen Daten stehen folgende Analysen zur Verfügung:
  """
    )
  ...
  """
    ...
    if uploaded_file is not None:
        ...
        status_slot.markdown(
            "<font color='green'>Daten sind geladen</font>", unsafe_allow_html=True
        )
    ...
    if col1.checkbox("Datensatz anzeigen"):
        if data_loaded:
            ...
            status_slot.markdown(
                "<font color='red'>Daten müssen zuerst geladen werden</font>",
    ...
    if col1.checkbox("Top Produktkäufe"):
        if data_loaded:
            top = col1.number_input(
                "Die Top-N Produktkäufe", min_value=1, max_value=100, value=10
            )
    ...
    col1.markdown(
        "## 5. Top-10 Produkte innerhalb der Länder mit den meisten Produktkäufen"
    )
        ...
        if data_loaded:
            ...
            with col1:
                for i, c in enumerate(stqdm(top_10_countries, desc="Erzeuge Plots")):
                    top_10_products = product_country_hist[
                        product_country_hist["Country"] == c
                    ].head(10)
                    sns.barplot(
                        x="Quantity",
                        y="Description",
    ...
    if col1.checkbox("Analyse durchführen"):
        if data_loaded:
            ...
            sns.lineplot(
                x="Month",
                y="Quantity",
                data=df.groupby("Month").sum("Quantity"),
                marker="o",
            )
            ...
            sns.lineplot(
                x="Day",
                y="Quantity",
                data=df.groupby("Day").sum("Quantity"),
                marker="o",
            )
    ...
    if col1.checkbox("RFM Analyse"):
        col1.markdown(
            """__RFM__ steht für __Recency__ (Aktualität), __Frequency__ und __Monetary__ Score derKunden
...
* __Monetary__: Gibt die Gesamtausgaben eines Kunden an
    ...
    if col1.checkbox("Erstelle Cluster"):
        if data_loaded:
            clusters = col1.number_input(
                "Anzahl Cluster", min_value=2, max_value=10, value=5
            )
            iterations = col1.number_input(
                "Anzahl Iterationen", min_value=10, max_value=1000, value=50
            )
            ...
            rfm_cluster_df = pd.concat(
                [rfm_summary, pd.Series(kmeans_model.labels_)], axis=1
            )
            rfm_cluster_df.columns = [
                "CustomerID",
                "Frequency",
                "Recency",
                "T",
    ...
    if col1.checkbox("Beta Geo Model"):
        if data_loaded:
            predicted_days = col1.number_input(
                "Anzahl Tage für Vorhersage", min_value=1, max_value=30, value=30
            )
            penalizer_coef = col1.number_input(
                "Penalizer Koeffizient", min_value=0.0, max_value=1.0, value=0.0
            )
